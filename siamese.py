...
def siamese_loss(out1, out2, y, Q=2.5):
...
def siamese(inputs, keep_prob, batch):
    # conv1+relu1
    # batch = tf.constant(batch, name="batch", dtype=tf.int32)
    with tf.name_scope('conv1') as scope:
        a = tf.random_normal_initializer
        # w1 = tf.Variable(tf.initializers.TruncatedNormal(stddev=0.05)(shape=[3, 3, 1, 32]), name='w1')
        w1 = tf.Variable(tf.truncated_normal(shape=[2, 2, 1, 32], stddev=0.05), name='w1')
    ...
    with tf.name_scope('conv2') as scope:
        ...
        w2 = tf.Variable(tf.truncated_normal(shape=[2, 2, 32, 64], stddev=0.05), name='w2')
        b2 = tf.Variable(tf.zeros(64), name='b2')
        conv2 = tf.nn.conv2d(relu1, w2, strides=[1, 1, 1, 1], padding='SAME', name='conv2')
    ...
    #     w3 = tf.Variable(tf.truncated_normal(shape=[3, 3, 64, 256], mean=0, stddev=0.05), name='w3')
    ...
    with tf.name_scope('fc1') as scope:
        x_flat = tf.reshape(relu2, shape=[-1, 9 * 64])
        w_fc1 = tf.Variable(tf.truncated_normal(shape=[9 * 64, 2], stddev=0.05, mean=0), name='w_fc1')
        b_fc1 = tf.Variable(tf.zeros(2), name='b_fc1')
    ...
    #     drop_2 = tf.nn.dropout(relu_fc1, keep_prob=keep_prob, name='drop_2')
    # with tf.name_scope('bn_fc2') as scope:
    #     bn_fc2 = tf.layers.batch_normalization(drop_2, name='bn_fc2')
    # with tf.name_scope('fc3') as scope:
    #     w_fc3 = tf.Variable(tf.truncated_normal(shape=[256, 2], stddev=0.05, mean=0), name='w_fc3')
