...
def siamese(inputs, keep_prob, batch):
    ...
    with tf.name_scope('fc1') as scope:
        x_flat = tf.reshape(relu3, shape=[-1, batch * 128])
        print(batch,keep_prob)
        w_fc1 = tf.Variable(tf.truncated_normal(shape=[batch * 128, 512], stddev=0.05, mean=0), name='w_fc1')
    ...
    with tf.name_scope('fc2') as scope:
        w_fc2 = tf.Variable(tf.truncated_normal(shape=[512, 2], stddev=0.05, mean=0), name='w_fc2')
